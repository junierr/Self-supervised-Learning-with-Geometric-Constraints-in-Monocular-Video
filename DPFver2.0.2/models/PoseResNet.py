...
class PoseDecoder(nn.Module):
    def __init__(self, num_ch_enc, num_input_features=1, num_frames_to_predict_for=1, stride=1):
        ...
        self.convs_squeeze = nn.Conv2d(self.num_ch_enc[-1], 256, 1)
        self.convs_pose0 = nn.Conv2d(num_input_features * 256, 256, 3, stride, 1)
        self.convs_pose1 = nn.Conv2d(256, 256, 3, stride, 1)
        self.convs_pose2 = nn.Conv2d(256, 6 * num_frames_to_predict_for, 1)
    ...
    def forward(self, input_features):
        ...
        cat_features = [self.relu(self.convs_squeeze(f)) for f in last_features]
#        cat_features = [self.relu(self.convs["squeeze"](f)) for f in last_features]
        cat_features = torch.cat(cat_features, 1)

        out = cat_features
        out = self.convs_pose0(out)
        out = self.relu(out)
        out = self.convs_pose1(out)
        out = self.relu(out)
        out = self.convs_pose2(out)
